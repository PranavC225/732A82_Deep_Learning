...
def scaled_dot_product_attention(q, k, v):
    ...
    # or Q @ tf.transpose(K, perm=[0,1,3,2])

    # Scale the dot products by the square root of the depth.
    dk = tf.math.sqrt(tf.cast(tf.shape(k)[-1], dtype=tf.float32)) # Shape: (batch_size, num_heads,)
...
class MultiHeadAttention(layers.Layer):
    def __init__(self, n_heads, proj_size, dk, dv, return_attention_weights=False):
        ...
        self.WQ = tf.Variable(initial_value=initializer(shape=(n_heads, dk, proj_size)), trainable=True, name='WQ')  # of shape (n_heads, proj_size, dq=dk)
        self.WK = tf.Variable(initial_value=initializer(shape=(n_heads, dk, proj_size)), trainable=True, name='WK')  # of shape (n_heads, proj_size, dk=dq)
        self.WV = tf.Variable(initial_value=initializer(shape=(n_heads, dv, proj_size)), trainable=True, name='WV')  # of shape (n_heads, proj_size, dv)
        self.WO = tf.Variable(initial_value=initializer(shape=(n_heads*proj_size,proj_size)), trainable=True, name='WO')  # of shape (n_heads*proj_size, proj_size)
    ...
    def call(self, Q, K, V):
        ...
        Qh = tf.transpose(Qh,[0,2,1,3])  # of shape (batch_size, H, number_of_Q, proj_size)
        Kh = tf.transpose(Kh,[0,2,1,3])  # of shape (batch_size, H, number_of_K, proj_size)
        Vh = tf.transpose(Vh,[0,2,1,3])  # of shape (batch_size, H, number_of_V, proj_size)
        ...
        A = tf.transpose(attention_pooling_h,[0,2,1,3]) 
        A = tf.reshape(A, [tf.shape(A)[0], tf.shape(A)[1], -1])# of shape (batch_size, number_of_Q, n_heads*proj_dim)
        ...
        attention_weights_h = tf.reduce_mean(attention_weights_h, axis=1)
        # ============================================
...
def transformerBlock(x, num_heads, projection_dim, transformer_units, dropout_rate=0.1):
    ...
    attention_output = MultiHeadAttention(n_heads=num_heads, proj_size=projection_dim, dk=projection_dim, dv=projection_dim)(x,x,x)
...
class PatchExtractor(Layer):
    def __init__(self, patch_size=16):
        ...
        """
        super().__init__()#super(PatchExtractor, self).__init__()
    ...
    def call(self, images):
        ...
        patches = tf.reshape(patches,[batch_size, (patch_dims[1] * patch_dims[2]), patch_dims[3]])
...
def create_vit_classifier(
    ...
    num_patches = patches.shape[1] #num_patches = (input_shape[0] // patch_size) * (input_shape[1] // patch_size)
    ## Encode the patches using the PatchEncoder layer
    encoded_patches = PatchEncoder(num_patches=num_patches, projection_dim=embedding_proj_dim)(patches)
    ...
    for _ in range(transformer_layers):
        encoded_patches = transformerBlock(x=encoded_patches,num_heads=num_heads,projection_dim=msa_proj_dim,transformer_units=transformer_units,dropout_rate=msa_dropout_rate)
...
def test_MultiHeadAttention(seed=42):
    ...
    # TODO : check that the random seed works on several machines.
    if (
        abs(output_mean_difference) <= 1e-2
        and abs(output_std_difference) <= 1e-1
        and abs(attention_weights_mean_difference) <= 1e-2
        and abs(attention_weights_std_difference) <= 1e-1
    ):
